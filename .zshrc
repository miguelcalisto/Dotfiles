...
# Iniciar uma sessão tmux temporária ao abrir o terminal
if command -v tmux &> /dev/null && [ -z "$TMUX" ]; then
    # Gera um nome único para a sessão com base no PID do terminal
    SESSION_NAME="session_$(date +%s%N)" # nome único usando o timestamp
    tmux new-session -d -s "$SESSION_NAME"  # Cria a sessão tmux
    tmux attach-session -t "$SESSION_NAME"  # Conecta à sessão
    trap 'tmux kill-session -t "$SESSION_NAME"' EXIT  # Mata a sessão ao sair do terminal
fi
